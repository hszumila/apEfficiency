int plotEffL1L1_1p5(){
  ...
  //L1L1, 0.5mm cuts:
  TCut cut = "eleClY*posClY<0&&eleHasL1&&posHasL1&&uncP>0.8*1.056&max(eleTrkChisq,posTrkChisq)<30&&eleP<0.75*1.056&&min(eleMinPositiveIso+0.5*(eleTrkZ0-5*elePY/eleP)*sign(elePY),posMinPositiveIso+0.5*(posTrkZ0-5*posPY/posP)*sign(posPY))>0&&bscChisq<10&&bscChisq-uncChisq<5&&uncP<1.15*1.056&&max(eleMatchChisq,posMatchChisq)<10&&abs(eleP-posP)/(eleP+posP)<0.4&&posTrkD0<1.5&&abs(eleFirstHitY)>2.5&&abs(posFirstHitY)>2.5";
  ...
  //make output file
  TFile *fout = new TFile("L1L1_1p5/outputEffL1L1.root","RECREATE");

  //aprime masses
  const int nmass = 11;
  double mass[nmass] = {24,26,28,30,35,40,50,60,70,80,90};
  double massD[nmass] = {0.024,0.026,0.028,0.030,0.035,0.040,0.050,0.060,0.070,0.080,0.090};
  ...
  std::string pdf_file_name = "L1L1_1p5/vertexEffFitsL1L1.pdf";
  canvas->Update();
  ...
  for (int ii = 0; ii<nmass; ii++){
    ...
    cout<<"Eff at target\t"<<effAtTarget<<endl;
  ...
  for (int kk=0; kk<npar; kk++){
    ...
    gg->Draw("alp");
    cc[kk]->Update();
    cc[kk]->SaveAs(Form("L1L1_1p5/Par%d.C",kk));
    cc[kk]->Close();
